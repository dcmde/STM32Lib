...
ERROR_t MPU6050_Init(MPU6050_t *MPU6050) {
    ...
    error = MPU6050->ReadBuffer(MPU6050_PWR_MGMT_1, &data, 1);
    if (error != NO_ERROR) {
        return error;
    }
    data &= ~((1 << 5) | (1 << 6));

    error = MPU6050->WriteBuffer(MPU6050_PWR_MGMT_1, &data, 1);
    if (error != NO_ERROR) {
        return error;
    }

    // Set the gyroscope configuration. +- 1000�/s
    error = MPU6050->ReadBuffer(MPU6050_GYRO_CONFIG, &data, 1);
    if (error != NO_ERROR) {
        return error;
    }
    ...
    data &= ~(1 << 3);
    data |= (1 << 4);
    error = MPU6050->WriteBuffer(MPU6050_GYRO_CONFIG, &data, 1);
    if (error != NO_ERROR) {
        return error;
    }

    // Set the acceleration configuration. +- 8g
    error = MPU6050->ReadBuffer(MPU6050_ACCEL_CONFIG, &data, 1);
    if (error != NO_ERROR) {
        return error;
    }
    ...
    error = MPU6050->WriteBuffer(MPU6050_ACCEL_CONFIG, &data, 1);
    if (error != NO_ERROR) {
        return error;
    }
...
ERROR_t MPU6050_ReadAccel(MPU6050_t *MPU6050) {
    ...
    error = MPU6050->ReadBuffer(MPU6050_ACCEL_XOUT_H, data, 6);
    if (error != NO_ERROR) {
        return error;
    }
...
ERROR_t MPU6050_ReadGyro(MPU6050_t *MPU6050) {
    ...
    error = MPU6050->ReadBuffer(MPU6050_GYRO_XOUT_H, data, 6);
    if (error != NO_ERROR) {
        return error;
    }
