...
int32_t BME280_CompensateTemp(int32_t adc_T, BME280_CompParam* BME280_CompParamStruct){
	int32_t var1, var2, T;
	var1 = ((((adc_T >> 3) - ((int32_t)BME280_CompParamStruct->DIG_T1 << 1))) * ((int32_t)BME280_CompParamStruct->DIG_T2) ) >> 11;
	var2 = (((((adc_T >> 4) - ((int32_t)BME280_CompParamStruct->DIG_T1)) * ((adc_T >> 4) - ((int32_t)BME280_CompParamStruct->DIG_T1))) >> 12) * ((int32_t)BME280_CompParamStruct->DIG_T3)) >> 14;
	t_fine = var1 + var2;
	T = (t_fine * 5 + 128) >> 8;
	return T;
}
...
uint32_t BME280_CompensatePres(int32_t adc_P, BME280_CompParam* BME280_CompParamStruct){
	...
	var1 = (((BME280_CompParamStruct->DIG_P3 * (((var1>>2) * (var1>>2)) >> 13 )) >> 3) + ((((int32_t)BME280_CompParamStruct->DIG_P2) * var1)>>1))>>18;
...
uint32_t BME280_CompensateHum(int32_t adc_H, BME280_CompParam* BME280_CompParamStruct){
	...
	v_x1_u32r = (((((adc_H << 14) - (((int32_t)BME280_CompParamStruct->DIG_H4) << 20) - (((int32_t)BME280_CompParamStruct->DIG_H5) * v_x1_u32r)) + \
							((int32_t)16384)) >> 15) * (((((((v_x1_u32r * ((int32_t)BME280_CompParamStruct->DIG_H6)) >> 10) * (((v_x1_u32r *((int32_t)BME280_CompParamStruct->DIG_H3)) >> 11) \
							+ ((int32_t)32768))) >> 10) + ((int32_t)2097152)) *((int32_t)BME280_CompParamStruct->DIG_H2) + 8192) >> 14));
	v_x1_u32r = (v_x1_u32r - (((((v_x1_u32r >> 15) * (v_x1_u32r >> 15)) >> 7) * ((int32_t)BME280_CompParamStruct->DIG_H1)) >> 4));
...
void BME280_ReadRegisters(const uint8_t register_address, 
													uint8_t* buffer, 
													const uint8_t size){
	i2c_read_buffer(I2C_ADDR, register_address, buffer, size);
}
...
void BME280_WriteRegister(uint8_t register_address, uint8_t value){
	i2c_write_buffer(I2C_ADDR, register_address, &value, 1);
}
